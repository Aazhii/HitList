...
/**
 * Create realistic seed notification records for first-run demo.
 * Called only when localStorage has no existing records.
 */
export function createSeedNotifications(todos: Todo[]): NotificationRecord[] {
  const nowMs = Date.now();
  const records: NotificationRecord[] = [];

  // Find the seed reminder tasks by id prefix
  const remind1 = todos.find((t) => t.id === 'seed-remind-1');
  const remind2 = todos.find((t) => t.id === 'seed-remind-2');
  const remind3 = todos.find((t) => t.id === 'seed-remind-3');

  if (remind1) {
    records.push({
      id: 'seed-notif-1',
      taskId: remind1.id,
      taskText: remind1.text,
      quadrant: remind1.quadrant,
      type: 'upcoming',
      triggeredAt: nowMs - 2 * 60 * 1000, // triggered 2 min ago
      dismissed: false,
      minutesBefore: 8,
      seenInToast: false,
    });
  ...
  if (remind2) {
    records.push({
      id: 'seed-notif-2',
      taskId: remind2.id,
      taskText: remind2.text,
      quadrant: remind2.quadrant,
      type: 'missed',
      triggeredAt: nowMs - 2.5 * 60 * 60 * 1000, // triggered 2.5h ago
      dismissed: false,
      seenInToast: false,
    });
  }
  ...
  if (remind3) {
    records.push({
      id: 'seed-notif-3',
      taskId: remind3.id,
      taskText: remind3.text,
      quadrant: remind3.quadrant,
      type: 'upcoming',
      triggeredAt: nowMs - 5 * 60 * 1000, // triggered 5 min ago
      dismissed: false,
      minutesBefore: 25,
      seenInToast: false,
    });
