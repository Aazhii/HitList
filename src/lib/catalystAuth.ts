 ...
 *
 * css_url REPLACES Catalyst's stylesheet rather than cascading on top of it,
 * and their sheet is what drives the form's state machine — it carries the
 * display:none rules that hide the steps which are not current (#otp_container,
 * #captcha_container, #recoverOption, .fielderror, .backbtn and the MFA
 * containers). An earlier theme-only sheet dropped those, so the iframe
 * rendered the email step, OTP entry, "Forgot Password?" and "Change" all at
 * once, stacked and overflowing.
 *
 * public/catalyst-login.css is therefore Catalyst's published base sheet
 * verbatim with our theme appended after its last line, which is what their
 * docs prescribe. Regenerate it with scripts/build-login-css.sh; never hand-edit
 * the part above the theme block.
 */
