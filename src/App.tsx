...
import { markEmailVerified, resendVerification, getVerifyToken } from '@/lib/localAuth';

function EmailVerificationBanner() {
  const { session, refreshSession } = useCatalystUser();
  ...
  // On mount, load the initial verifyToken from the user record
  const initialTokenLoaded = useState(() => {
    if (session && !session.emailVerified) {
      const t = getVerifyToken(session.userId);
      if (t) setTokenDisplay(t);
    }
    return true;
  })[0];
  void initialTokenLoaded;

  if (!session || session.emailVerified || dismissed) return null;

  function handleVerifyDirect() {
    if (!session) return;
    markEmailVerified(session.userId);
    refreshSession();
    toast.success('Email verified! Your account is now fully active.', { duration: 3000 });
  }

  function handleVerifyWithToken() {
    if (!session) return;
    setTokenError(null);
    const trimmed = inputToken.trim();
    if (!trimmed) { setTokenError('Please enter your verification token.'); return; }
    // Check against stored token
    const stored = getVerifyToken(session.userId);
    if (!stored || trimmed !== stored) {
      setTokenError('Invalid token. Please check and try again, or request a new one.');
      return;
    }
    setVerifying(true);
    markEmailVerified(session.userId);
  ...
  function handleResend() {
    if (!session) return;
    setResending(true);
    const res = resendVerification(session.userId);
    setResending(false);
    if (res.ok === true) {
      setTokenDisplay(res.token);
      setInputToken('');
      setTokenError(null);
      toast('New verification token generated.', { duration: 2000 });
    }
  }
  ...
  return (
    <div className="mx-4 md:mx-6 mt-3 rounded-xl border border-yellow-500/30 bg-yellow-500/8 px-4 py-3.5 space-y-3 animate-fade-in">
      {/* Header row */}
      <div className="flex items-start gap-3">
        <ShieldAlert className="size-4 text-yellow-500 flex-shrink-0 mt-0.5" />
        <div className="flex-1 min-w-0">
          <p className="text-xs font-semibold text-foreground">Verify your email address</p>
          <p className="text-[11px] text-muted-foreground mt-0.5 leading-relaxed">
            Your account is active but unverified. Verify to unlock all features.
          </p>
        </div>
        ...
        <div className="rounded-lg border border-border bg-background/60 p-3 space-y-2">
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-1">
              ...
              >
                {copied ? (
                  <><MailCheck className="size-3 text-primary" /><span className="text-primary">Copied</span></>
                ) : (
                  <><MailCheck className="size-3" />Copy</>
                )}
              </button>
          ...
          <p className="text-[10px] text-muted-foreground">
            In a real app this would be emailed to you. Click <strong>Verify now</strong> above, or enter the token below to verify manually.
          </p>
        </div>
      )}
      ...
      <div className="flex items-start gap-2">
        <button
          ...
          className="text-[11px] text-muted-foreground hover:text-foreground transition-colors duration-150 underline underline-offset-2"
        >
          {showTokenInput ? 'Hide token entry' : 'Enter token manually'}
        </button>
      </div>
        ...
        <div className="space-y-2 animate-fade-in">
          <div className="flex gap-2">
            <input
              type="text"
              value={inputToken}
              onChange={(e) => { setInputToken(e.target.value); setTokenError(null); }}
              placeholder="Paste your verification token…"
              className="flex-1 h-8 rounded-lg border border-border bg-background px-3 text-[11px] font-mono text-foreground placeholder:text-muted-foreground/50 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-1"
            />
            <Button
              size="sm"
              variant="outline"
          ...
          {tokenError && (
            <p className="text-[11px] text-destructive animate-fade-in">{tokenError}</p>
          )}
        </div>
      )}
    </div>
